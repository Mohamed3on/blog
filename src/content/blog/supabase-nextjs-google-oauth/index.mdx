...
description: "A comprehensive guide to adding Supabase Google OAuth in Next.js (App Router) with dynamic origins, wildcards, and environment considerations."
...
**Mismatch between Supabase settings and Google Cloud settings is a very common source of OAuth errors!** They must align.
...
- Your local `.env.local` file (or equivalent).
- Your hosting provider's settings (e.g., Vercel Environment Variables) for Production, Preview, and Development environments.

Missing or incorrect variables will prevent the Supabase client from initializing correctly.
...
This table stores public user profile information, linked to the `auth.users` table.
...
-- Allow individual user update access to their own profile
...
- The `id` column links directly to `auth.users.id`.
- RLS is enabled to control access.
- Policies allow public reads and owner-only updates (adjust as needed).

#### Step 2: Create a Trigger to Populate `profiles`

This trigger automatically creates a profile entry when a new user is added to `auth.users`. It copies the user ID and attempts to extract the `full_name` and `avatar_url` from the metadata Supabase saves during the OAuth flow (`raw_user_meta_data`).
...
- The `handle_new_user` function performs the `insert` into `public.profiles`.
- It uses data from the `new` row being inserted into `auth.users`.
- The `on_auth_user_created` trigger executes this function automatically after user creation.
